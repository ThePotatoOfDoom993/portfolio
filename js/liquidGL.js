...
(() => {
  ...
  const NaughtyDOM = (() => {
    ...
    Painter.prototype.replaced = function (node) {
      const el = node.el;
      const tag = el.tagName;
      if (tag === "VIDEO" || tag === "IFRAME" || tag === "CANVAS") return;
  ...
  class liquidGLRenderer {
    ...
    async captureSnapshot() {
      ...
      const attemptCapture = async (
        ...
        try {
          ...
          console.warn("liquidGL: Snapshot capture notice. Using CSS backdrop-filter fallback.");
          this._applyCssFallback();
          return false;
        } finally {
    ...
    _uploadTexture(srcCanvas) {
      ...
      try {
        ...
        console.warn("liquidGL: Tainted canvas in texImage2D (file:// or CORS origin). Using CSS glassmorphism fallback.");
        this._applyCssFallback();
        return false;
      }
  ...
  window.liquidGL.syncWith = function (config = {}) {
    const renderer = window.__liquidGLRenderer__;
    if (!renderer) {
      return { lenis: null, locomotiveScroll: null };
    }
