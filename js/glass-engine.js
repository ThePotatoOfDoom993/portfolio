(function() {
  ...
  vec4 getBackgroundWithPositions(vec2 uv) {
      ...
      // Smooth vertical gradient transparency fade so it doesn't end abruptly
      float fade = smoothstep(0.0, 0.8, uv.y);
      col = mix(u_bg_color_1, col, fade);
  ...
  function updateTrackedElements() {
    const now = Date.now();
    if (now - lastDomQueryTime < 1000 && trackedElements.length > 0) return;
    lastDomQueryTime = now;

    const selectors = '.glass-panel, .nav-shell, .chart-frame, .dial-panel, .rules-panel, .float-card, .quote-card, .config-card';
    ...
    trackedElements = list.slice(0, 15);
  }
    ...
    for (let i = 0; i < trackedElements.length; i++) {
      ...
      if (document.body.contains(item.el)) {
        ...
        if (rect.bottom > 0 && rect.top < window.innerHeight && rect.right > 0 && rect.left < window.innerWidth) {
          panels.push({ rect, radius: item.radius });
        }
      }
    }
    return panels;
  ...
  function render() {
    ...
    // Real-time panel boundary tracking
